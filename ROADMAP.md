...
1. extend structured diagnostics to report/query failures, proof/evidence failures, artifact/registry failures, package/interface failures, and backend-contract failures (core pipeline phases — parse through SSA verify — are fully converted; `liftStringError` is eliminated)
