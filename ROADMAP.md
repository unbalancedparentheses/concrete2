...
25. design the standard small-systems programming surface and first complete stdlib target for the stable subset: fixed-capacity buffers/strings, binary parsing helpers, deterministic cleanup helpers, small FFI-wrapper patterns, the exact modules/APIs that are in scope for the first supported use cases, what is intentionally out of scope, which surfaces are borrowed from Rust/Zig/Austral or other influences, and which modules must remain proof/predictability-friendly versus hosted-only
26. return to stdlib/example polish with that target in mind: split/trim, path decomposition, minimal FFI pressure test, remove accidental API sprawl, and make the stable-subset stdlib feel complete rather than exploratory
