...
**STEP 2 NEXT: attest the remaining tables**, one table per step so a broken reduction stays attributable to the table that caused it. `cryptoFns` is 1 of the 9 the hand-back names; `FnTable.empty` is now exempt by emptiness rather than needing conversion.
