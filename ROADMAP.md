...
**Invariants held across step 1:** manifest 86 = 0 + 38 + 48, reuse 13 packages, correspondence 9/10 + 1 no-claim, globals still dispatch, `cryptoFns` entries unchanged (4). Gates: build clean, DEPENDENCY-EDGES 273/0, ATTESTATION-MANIFEST 13/0, CLASSIFICATION-FRESHNESS 2/0, SUBJECT-FACTS 85/0, IMPL-MANIFEST-GATE 8/0, ONE-PRODUCER 12/0, DOCS-DRIFT 3/0.
...
- **The denominator does not RECONCILE.** It reports `source_linked_theorems = 31` against 43 unique source-linked names; the gap is theorems in `.con` files outside any directory holding a `Concrete.toml`. Reporting two numbers is not reconciling them.
- **In-repo theorems have no package.** `proofFns`, `proofFnsExt` and `FnTable.empty` map only to in-repo theorems, so no source package is determined. The manifest states this rather than guessing, but those tables cannot be converted until it is resolved — and `FnTable.empty` is the 8-package table, so it is the highest-risk one.
- **Only empty-population refusals exist.** Duplicate, ambiguous, conflicting and surplus mappings are not refused, no mutation control exists, and identities are not yet recomputed against current compiler facts.

**ORIGINAL ACCEPTANCE CONDITIONS, ratified 2026-08-14 (unchanged, still governing):** Produces an exact manifest keyed by TABLE and SCOPED IMPLEMENTATION, derived from BOTH attachment populations (source-linked proof attributes/registry, and Lean-side theorem/table usage). It must:
...
| **dispatch is security-relevant inventory — 3 of 5 controls in place** | HAS: one canonical owner (`TableResolve.lean`), completeness against every hand-back name (8/9, gated), duplicate-name refusal (gated — a duplicate arm wins silently otherwise). **MISSING: stable table identity verification, and a mutation proving a removed/misrouted entry makes the subject refuse.** Identity verification is not small: the hand-back digest is `shortHash ("tableV1:" ++ name ++ toString v)` over the SYNTACTIC term at generation time, which the compiler cannot reproduce at runtime. It needs a digest the compiler CAN recompute — over entry identities and body digests — which the hand-back does not carry. **That, not `evalExpr`, is the real remaining hand-back gap** | add an entry-derived table digest to the hand-back and compare it; register the misroute mutation |
