...
5. validate the effect taxonomy in practice — done when the reported operational/trust effects are useful on real examples without collapsing into noise
6. validate failure semantics for restricted profiles — done when the first profile has an explicit answer for abort, assertions, impossible branches, and excluded OOM paths
7. validate fixed-capacity stdlib viability — done when at least one bounded no-allocation example uses a realistic fixed-capacity or equivalent restricted-data-structure approach cleanly
8. define the Concrete-to-Lean proof pipeline — done when the compiler has a clear extraction and traceability story from checked Concrete into Lean-proof artifacts
9. define how specifications attach to Concrete functions — done when one function, one spec, and one theorem can be linked without ambiguity
10. define a useful provable systems subset — done when the project can name a restricted but still meaningful class of no-GC systems functions that fit the proof workflow
11. prove selected user-facing functions and report properties — done when the project can demonstrate end-to-end proof-backed evidence on a meaningful restricted fragment
12. define the backend/source trust boundary for evidence claims — done when it is explicit which claims hold at source/report level and where backend assumptions begin
13. validate the analyzable-concurrency stance — done when the project has a concrete answer on whether thesis-level validation remains single-threaded or admits a restricted concurrency subset
14. validate the thesis with bounded and evidence-carrying examples — done when a small set of examples demonstrates capability visibility, predictable execution, and proof-backed evidence together
