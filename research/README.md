...
8. [proof-evidence/operational-vc-auto-discharge.md](proof-evidence/operational-vc-auto-discharge.md) — generating operational VCs from Core/ProofCore so common postconditions and loop steps can discharge through `omega` / `bv_decide` before asking for hand Lean (`P0`, `Open`)
9. [compiler/performance-research-packets.md](compiler/performance-research-packets.md) — agent-readable performance/evidence packets for human or LLM optimization research (`P1`, `Open`)
10. [language/memory-ub-boundary.md](language/memory-ub-boundary.md) — memory/UB classification for predictable and proof-backed evidence claims (`P1`, `Open`)
11. [compiler/backend-traceability.md](compiler/backend-traceability.md) — source/Core/SSA/LLVM/machine behavior traceability (`P1`, `Open`)
12. [language/failure-semantics.md](language/failure-semantics.md) — failure model for abort, assertion failure, impossible paths, OOM, and restricted profiles (`P1`, `Open`)
13. [language/trusted-code-policy.md](language/trusted-code-policy.md) — trusted-code audit policy and restricted-profile behavior (`P1`, `Open`)
14. [proof-evidence/formalization-roi.md](proof-evidence/formalization-roi.md) — what to prove first and why (`P0`, `Open`)
15. [proof-evidence/formalization-breakdown.md](proof-evidence/formalization-breakdown.md) — the full formalization effort split into tracks and milestones (`P0`, `Open`)
...
- [proof-evidence/operational-vc-auto-discharge.md](proof-evidence/operational-vc-auto-discharge.md) — operational VC generation and kernel-decision auto-discharge for common postconditions and loop steps (`P0`, `Open`)
