...
cc() {
  ...
  local anchor=""
  case "$rep" in
    subject-facts)             anchor='=== Subject facts' ;;
    proof-status)              anchor='=== Proof Status Report' ;;
    consistency)               anchor='consistency check|consistency violation' ;;
    generated-implementations) anchor='GENERATED by' ;;
    *)                         anchor='' ;;
  esac
  if [ -z "$out" ]; then
    printf '%s --report %s produced NO output (exit %s)\n' "$f" "$rep" "$rc" >> "$CC_FAILURES"
  elif [ -z "$anchor" ]; then
    printf '%s --report %s has no declared structural anchor (exit %s)\n' "$f" "$rep" "$rc" >> "$CC_FAILURES"
  elif ! grep -qE -- "$anchor" <<<"$out"; then
    printf '%s --report %s output lacks its anchor /%s/ — truncated or not the report asked for (exit %s)\n' \
      "$f" "$rep" "$anchor" "$rc" >> "$CC_FAILURES"
  fi
  printf '%s' "$out"
}
