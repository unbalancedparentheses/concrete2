...
declare -A SKIP_BUGS=(
  ...
  [071]="OPEN (PARTIALLY REPAIRED 2026-09-21) -- a capability declared in a DEPENDENCY did not bind on its caller: a function declaring nothing called std.env.get (with Env, Alloc, Unsafe) and the program printed \$HOME. Only the hardcoded intrinsic table was enforced across a package, so every std with(...) was decorative to consumers -- println looked enforced ONLY because it is an intrinsic. STAGE 1: sink capabilities and Alloc now bind for free functions and methods alike, via CModule.importedFnCaps keyed by local spelling (get is declared in ten std modules, so a bare-name table would cross-attach). Unsafe is deliberately NOT yet enforced cross-package: doing so gave 91 diagnostics over 32 of 95 packages, 82 missing only Unsafe with the caller holding the full Std set, which excludes Unsafe by definition. Gated by check_cross_package_caps.sh (16/0) with two compiler mutations; the exception is pinned there so removing it fails the gate. STAYS OPEN until the 82 are classified and the exception removed; see docs/bugs/071"
