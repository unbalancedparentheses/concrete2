...
declare -A SKIP_BUGS=(
  ...
  [065]="OPEN -- builtinEnumList built twice AND hasUserResult computed from different inputs (Elab consults imports, Check does not), so the passes hold different beliefs about a user Result; code divergence measured, no wrong-behaviour witness yet"
