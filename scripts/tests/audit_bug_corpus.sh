...
declare -A SKIP_BUGS=(
  ...
  [065]="OPEN -- --report stack-depth drops recursive callees from each caller's call graph, so a bounded caller of unbounded code reports a finite stack and the summary prints it as the program maximum. Report-only (stackBound has one consumer, the report itself; reaches no claim record or policy), and unreachable under --check predictable, which refuses recursion. Regression vehicle is check_stack_depth.sh (does not exist yet) over a direct+mutual recursion fixture, not a tests/programs .con"
