...
declare -A SKIP_BUGS=(
  ...
  [067]="OPEN — a capped obligation escapes forbid-assume when Rocq/Isabelle are present: the cap is display-only, so an obligation resting on an UNSOUND HYPOTHESIS passes the release gate. Asserted by check_known_wrong_corpus.sh, which fails in CI (and only there — no external provers locally). Authority leak; see docs/bugs/067"
