...
declare -A SKIP_BUGS=(
  ...
  [071]="FIXED 2026-09-21 (sinks+Alloc) / 2026-09-25 (Unsafe) -- a capability declared in a DEPENDENCY did not bind on its caller: a function declaring nothing called std.env.get and the program printed \$HOME. Only the hardcoded intrinsic table was enforced across a package, so every std with(...) was decorative to consumers -- println looked enforced ONLY because it is an intrinsic. FIX: CModule.importedFnCaps carries each imported callable's requirement keyed by the LOCAL call spelling (get is declared in ten std modules, so a bare-name table would cross-attach), consulted on the same decideCall path as local and sibling calls. Unsafe was held back one release, then landed via the std migration rather than an exemption: public Unsafe declarations 154->21, enabled by per-method trusted (67 of 154 do raw work in their OWN body, so safe-interface/audited-implementation had to become expressible). Gated by check_cross_package_caps.sh (18/0) incl. a with(Std) program that must BUILD AND RUN; see docs/bugs/071"
