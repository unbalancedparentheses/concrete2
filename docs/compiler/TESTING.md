...
Threat model defined in `docs/verification/THREAT_MODEL.md` covering 6 categories: proof semantic drift, authority escalation, validation weakening, resource drift, trust boundary erosion, specification mismatch.
...
That paragraph is the Phase-14 completion snapshot, not the current diagnostic
inventory. R-0004 containment later added the distinct `unbound` state and
E0810; current claim documents and report schemas include it separately.
