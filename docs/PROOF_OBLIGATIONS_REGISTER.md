...
| R-19 | `arraySet arr idx val` | `arr[i] = v` extracted as shadowing `letIn name (arraySet …)` | fixed_capacity (`ring_push`) | `array_set_preservation` + `eval_array_set_reduces` — **extraction + eval discharged 2026-05-30** (named in `docs/PROOF_STATE_MODEL.md` § 2); OOB stuck |
