...
8 new drift-detection gates in CI evidence section verify: trust weakening detected, `proved → stale` transitions, `is_pure: true → false`, File capability escalation, unbounded loop drift. 24 proof-pressure-set gates verify all 6 obligation states, extraction forms, extraction status/blockers, obligation dependencies/sources, eligibility reasons, effects/evidence consistency, and fingerprint/extraction determinism against `examples/proof_pressure/`. Trust-gate: 1115 checks, now includes 27 malformed-artifact attack tests (4 new: registry missing-field warnings, diff `<missing>` sentinel, unknown fact kind drift, empty function warning), 13 state-desynchronization attack tests, 11 invalid-query diagnostic tests, 9 API versioning envelope tests, 7 error code taxonomy tests, 14 policy enforcement adversarial tests, 24 proof pressure set inspection gates, and bug corpus audit.
