...
| shl (left shift) | Supported at `u32` (unsigned view, TRUNCATES to width, models LLVM `shl`), forced by HMAC-SHA256's `rotr`. Other widths are append-only follow-ups. |
...
3. an entry in `docs/verification/PROOF_OBLIGATIONS_REGISTER.md`,
...
- `docs/verification/PROOF_STORY_MATRIX.md` is the per-construct proof/evidence inventory.
- `docs/verification/PROVABLE_SUBSET.md` is the broader background document.
- `docs/verification/PROOF_OBLIGATIONS_REGISTER.md` records per-construct soundness debt.
- `docs/verification/PROOF_STATE_MODEL.md` defines the functional mutation model.
- `docs/verification/CLAIM_TAXONOMY.md` defines the claim vocabulary.
- `docs/verification/THREAT_MODEL.md` explains what the evidence pipeline catches.
- `docs/verification/CLAIMS_TODAY.md` is the public claim summary.
