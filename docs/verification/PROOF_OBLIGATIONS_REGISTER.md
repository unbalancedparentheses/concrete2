...
| R-07 | `ifThenElse c t e` | `if c { t } else { e }` and `if c { return X; } else-fallthrough` | parse_validate | `if_no_else_as_fallthrough_preservation` + `eval_if_true` + `eval_if_false` — **extraction + eval discharged 2026-05-30**; full if-else extraction forms still open |
| R-08 | `call fn args` | function call (FnTable lookup) | parse_validate | `call_preservation` + `eval_call_reduces` — **extraction + eval discharged 2026-05-30** |
| R-09 | `structLit name fields` | `Point { x: 1, y: 2 }` | parse_validate (parse_header Ok) | `struct_lit_preservation` + `eval_struct_lit_reduces` — **extraction + eval discharged 2026-05-30** |
| R-10 | `fieldAccess obj f` | `obj.f` | parse_validate | `field_access_preservation` + `eval_field_access_struct` + `eval_field_access_enum` — **extraction + eval discharged 2026-05-30** |
| R-11 | `enumLit ename var fields` | `Result::Ok { value: v }` | parse_validate (parse_header Err) | `enum_lit_preservation` + `eval_enum_lit_reduces` — **extraction + eval discharged 2026-05-30** |
| R-12 | `match_ scrutinee arms` (`PMatchPat`: `enumPat` / `litPat` / `varPat`) | `match c { Pat => body, ... }` | parse_validate (`error_code`) | `match_preservation` + `eval_match_reduces` — **extraction + eval discharged 2026-05-30**; per-arm preservation is a separate follow-up |
| R-13 | `arrayIndex arr idx` | `arr[i]` (read) | parse_validate (compute_checksum) | `array_index_preservation` + `eval_array_index_reduces` — **extraction + eval discharged 2026-05-30**; OOB returns `none` |
| R-14 | `cast inner` (identity on `PVal.int`) | `x as i32`, `u8 as i32` | fixed_capacity (`read_u8`, `read_u16_be`) | `cast_identity_preservation` + `eval_cast_identity` — **extraction + eval discharged 2026-05-30**; sound for widening; narrowing is excluded by eligibility |
| R-15 | `arrayLit elems` | `[0, 0, 0, ...]` | fixed_capacity (`ring_new`) | `array_lit_preservation` + `eval_array_lit_reduces` — **extraction + eval discharged 2026-05-30** |
| R-16 | `binOp (.mod width signed)` (BitVec.srem / BitVec.umod) | `a % b` for `i32`/`u32` (other widths reject at extraction) | fixed_capacity (`ring_push`'s `head % cap`) | extraction **subsumed by R-04** (parametric over `pop`); BitVec-vs-LLVM-srem semantic agreement is a separate eval-side obligation (open) |
| R-17 | `binOp (.bitxor width signed)` (BitVec.xor at width; signed/unsigned result interpretation) | `a ^ b` for `i32`/`u32`/`u8` (other widths reject at extraction) | fixed_capacity (`compute_tag`), parse_validate (`compute_checksum`), constant_time_tag (`ct_compare`) | extraction **subsumed by R-04**; BitVec-vs-LLVM-xor semantic agreement is open |
| R-21 | `binOp (.bitor width signed)` (BitVec.or at width; signed/unsigned result interpretation) | `a \| b` for `u8` (other widths reject at extraction) | constant_time_tag (`ct_compare` OR-accumulate) | extraction **subsumed by R-04**; BitVec-vs-LLVM-or semantic agreement is open |
| R-18 | `while_ cond assigns cont` (flat-assign body) | `while cond { x = ...; y = ...; }` | parse_validate (`compute_checksum`), fixed_capacity (`compute_tag`) | `while_flat_preservation` + `eval_while_false` + `eval_while_true_step` — **extraction + eval discharged 2026-05-30**; termination via fuel only — `bounded` profile enforces real termination at Check |
| R-19 | `arraySet arr idx val` | `arr[i] = v` extracted as shadowing `letIn name (arraySet …)` | fixed_capacity (`ring_push`) | `array_set_preservation` + `eval_array_set_reduces` — **extraction + eval discharged 2026-05-30** (named in `docs/verification/PROOF_STATE_MODEL.md` § 2); OOB stuck |
| R-20 | `while_step cond carried step cont` + `LoopStep::Cont`/`Break` enum | `while cond { let x = ...; if c { return v; } i = i + 1 }` | fixed_capacity (`ring_contains`) | `while_step_preservation` — **extraction discharged against `cStmtsToPExprK` 2026-05-30** (the fallback branch when `cAssignBodyToUpdates body = none`); the `while_step_early_break` eval-side claim (Cont/Break enum dispatch) is open as a separate obligation |
...
|---:|---|---|---|
| G-01 | Body fingerprint check | `hash` of the source `CExpr/CStmt` fingerprint matches the in-source `#[proof_fingerprint]`. Drift downgrades obligation status to `stale`; a source proof link with no stored hash is `unbound` (E0810), never proved. This is body-level containment, not the full signature/contract/dependency subject (R-0004). | `fingerprint_determinism`: source produces a single canonical fingerprint string; equivalent source produces same fingerprint. |
...
| G-03 | Linked-proof attachment | The in-source `#[proof_by]` references a Lean theorem by name; the Lean module containing the theorem is part of `make build` (Lean kernel rejection fails the build). | `theorem_lookup_completeness`: every `#[proof_by]` resolves to a Lean theorem; theorem statement uses the `#[spec]` symbol. |
...
| G-06 | Proof coverage classification (Phase 1 items 3+4, landed 2026-05-30) | Every registered proof carries a `coverage` field naming what KIND of theorem it is: `point` / `one_direction` / `iff` / `invariant` / `runtime_error` / `full_contract`.  Surfaced in `--report proof-status` and in `tests/showcase/manifest.toml`'s `[flagship.evidence].proof_coverage` per-flagship table.  A reviewer reading "X is proved" now sees the classification, so a single-instance point proof cannot pose as a universal claim.  Registry parser warns on unclassified entries. | `coverage_consistency`: every registered proof has a well-formed coverage tag and the displayed tag matches the actual theorem shape (no Lean-level enforcement; relies on author honesty + reviewer cross-check). |
...
state-model decision in `docs/verification/PROOF_STATE_MODEL.md` § 5
...
Pending extension rules — designed in `docs/verification/PROOF_STATE_MODEL.md`
...
- `docs/verification/PROOF_STATE_MODEL.md` — the state-model design.  This
  register is partly the contract surface that doc generates.
- `docs/verification/PROOF_AUDIT_PIPELINE.md` — the target proof pipeline.
