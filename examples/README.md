...
| [`fixed_point/`](fixed_point/) | runtime safety on a real-ish scalar pipeline: division and overflow machine-checked — incl. the nonlinear `sample * gain` via interval analysis + `bv_decide`; functional spec (exact result ranges) honestly pending. |
